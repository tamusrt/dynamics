...
%     fillOx(i) = data.inp.prp.fillOx;
    ...
    summ.railExit(j) = velMag(ind,j);
    end
    railExit(i) = mean(summ.railExit);
end
...
for i = 1:numel(z)
    try
        ix = find(x == fillOx(i+96));
        iy = find(y == presTank(i+96));
    catch
        continue
    end
    z(ix,iy) = apogee(i+96);   
...
%% Plot 
cStepApg = 2000:1000:30000;
cStepMach = 0:.05:2;
[c,h] = contourf(xq,yq,zq,cStepApg);
colormap(flipud(summer));
hold on;
plot([15, 25], [750, 850], 'Color', 'k', 'LineStyle','--') %set bounds
