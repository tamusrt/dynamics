...
function drawBars(spec, unit) {
  const tol = Math.pow(10, -spec.dec) / 2;
  ...
  for (const a of ALL) {
    ...
    const ok = a.rows.filter(r => r.ok && r.m[state.metric] != null);
    const pts = new Map();
    ok.forEach((r, i) => { if (!i) return; const v = val(r, state.metric); pts.set(r.short, { v, d: v - val(ok[i - 1], state.metric), r }); commits.set(r.short, r); });
    series.push({ a, pts });
  }
  ...
  if (!order.length) { chart = null; return; }
  const sets = series.map(({ a, pts }) => {
    const raw = order.map(r => pts.get(r.short) || null);
    const data = raw.map(p => p ? p.d : null);
    return { label: simLabel(a), data, raw, borderColor: colorOf.get(a.id), borderWidth: series.length > 1 ? 2 : 0, borderRadius: 3,
             backgroundColor: data.map(d => d == null || Math.abs(d) < tol ? css('--flat') : d > 0 ? css('--up') : css('--down')) };
  });
  chart = new Chart(cv, {
    type: 'bar', data: { labels: order.map(r => `${r.date.slice(5)} ${r.short}`), datasets: sets },
    options: {
      responsive: true, maintainAspectRatio: false, animation: false,
      onClick: (e, els) => { if (els.length && DATA.repo) { const r = order[els[0].index]; if (r.sha) window.open(`${DATA.repo}/commit/${r.sha}`, '_blank'); } },
      plugins: {
        legend: { display: sets.length > 1, position: 'top', labels: { boxWidth: 12,
          generateLabels: c => c.data.datasets.map((ds, i) => ({ text: ds.label, fillStyle: 'transparent', strokeStyle: ds.borderColor, lineWidth: 3, datasetIndex: i, hidden: !c.isDatasetVisible(i), fontColor: css('--fg') })) } },
        tooltip: { callbacks: {
          title: items => { const r = order[items[0].dataIndex]; return `${r.short} · ${r.date} · ${r.author}`; },
          label: item => { const p = item.dataset.raw[item.dataIndex]; if (!p) return null; const prev = p.v - p.d; const u = spec.unit ? ' ' + spec.unit : '';
            return [`${item.dataset.label}: Δ ${p.d >= 0 ? '+' : ''}${fmt(p.d, spec.dec)}${u}` + (prev ? ` (${(p.d / prev * 100).toFixed(1)}%)` : ''), `now ${fmt(p.v, spec.dec)}${u}`, p.r.message]; } } }
      },
      scales: {
        x: { grid: { display: false }, ticks: { maxRotation: 60, autoSkip: true }, title: { display: true, text: 'commit' } },
        y: { title: { display: true, text: 'Δ ' + spec.label + unit }, grace: '10%' }
      }
    }
  });
}
...
function draw() {
  ...
  const delta = state.view === 'dline';
  empty.textContent = 'Select simulations in the list.';
  const sets = [];
  for (const a of ALL) {
    if (!state.sel.has(a.id)) continue;
    const ok = a.rows.filter(r => r.ok && r.m[state.metric] != null);
    const pts = ok.map((r, i) => { const v = val(r, state.metric), d = i ? v - val(ok[i - 1], state.metric) : null;
                                   return { x: r.t * 1000, y: delta ? (d ?? 0) : v, v, d, r }; });
    const col = colorOf.get(a.id);
    sets.push({ label: simLabel(a), data: pts, borderColor: col, backgroundColor: col,
  ...
  chart = new Chart(cv, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { title: { display: true, text: (delta ? 'Δ ' : '') + spec.label + unit }, grace: '8%' }
      }
    }
  });
}
